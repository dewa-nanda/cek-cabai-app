...
<div class="min-h-screen" style="background-color: #F3F8FF">
    ...
    <section class="bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-gray-700 via-gray-900 to-black p-2">  
        ...
        <div class="px-4 mx-auto max-w-screen-xl text-center py-24 lg:py-20">
            <h1 class="mb-4 text-4xl font-extrabold tracking-tight leading-none text-white md:text-5xl lg:text-6xl">List Pakar Tanaman Cabai</h1>
            <p class="mb-8 text-lg font-normal text-gray-300 lg:text-xl sm:px-16 lg:px-48">Berikut merupakan list dari pakar tanaman cabai yang ada di UPT BP4 Wilayah VIII Prambanan</p>
    ...
    <div class="container mx-auto my-5 py-2" id="list">
        <div class="">
            <a href="{{route('addPakarView')}}" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-1.5 me-2 mb-2 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800">Tambah Pakar</a>
                        ...
                        @foreach ($user as $item)
                            <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                <th scope="row" class="px-6 py-4 font-medium text-gray-900 wrap dark:text-white max-w-24">
                                    <p class="line-clamp-3">{{$item->username}}</p>
                                </th>

                                <th class="px-6 py-4 text-left max-w-80">
                                    <p class="line-clamp-3">{{$item->email}}</p>
                                </th>

                                <th class="px-6 py-4 text-left max-w-80">
                                    <p class="line-clamp-3">{{$item->noHp}}</p>
                                </th>

                                <th class="px-6 py-4 text-left max-w-80">
                                    <p class="line-clamp-3">{{$item->type}}</p>
                                </th>

                                <th class="px-6 py-4 text-left flex gap-3 ">
                                    <a href="{{route('editPakarView', $item->id)}}" class="font-medium text-blue-600 dark:text-blue-500 hover:underline"><i class="fa-regular fa-pen-to-square text-2xl"></i></a>
