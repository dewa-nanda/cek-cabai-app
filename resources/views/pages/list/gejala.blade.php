    ...
    <nav class="mx-auto max-w-screen-xl flex ps-6 w-4/5" aria-label="Breadcrumb">
        <ol class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
            ...
            <li aria-current="page">
                ...
                <span class="ms-1 text-sm font-medium text-gray-500 md:ms-2 dark:text-gray-400 ">List Gejala</span>
                </div>
            </li>
        </ol>
    </nav>
    ...
    <div class="mx-auto max-w-screen-xl">
        <div class="grid justify-items-center xl:grid-cols-4 md:grid-cols-3 sm:grid-cols-1 gap-5 p-5">
            @foreach ($gejala as $item)
                <x-card title="{{$item->name}}" routeName="detailGejalaTanamanCabaiView" target="{{$item->id}}" desc="{{$item->description}}"/>
            @endforeach
        </div>
